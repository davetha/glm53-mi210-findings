...
# --- MLA projection-path timing (GLM53_MLAP_TIMING=<report interval>) ---
# The MLA layer's projection path measured 0.84 ms/call against a ~0.026 ms
# roofline (32x off) -- too slow for plain eager dispatch, so one op is slow
# rather than the launches. Marks are inline (no class wrapping: wrapping a
# shared class method once per layer nests the timings).
import os as _os_mp

_MLAP = int(_os_mp.environ.get("GLM53_MLAP_TIMING", "0"))
_MP = {"n": 0, "cur": [], "pend": [], "acc": {}}


def _mk(label: str) -> None:
    if not _MLAP:
        return
    import torch as _t
    e = _t.cuda.Event(enable_timing=True)
    e.record()
    _MP["cur"].append((label, e))
...
def _mk_flush() -> None:
    ...
    for seq in _MP["pend"]:
        ...
        for i in range(1, len(seq)):
            lbl = seq[i][0]
            _MP["acc"][lbl] = _MP["acc"].get(lbl, 0.0) + seq[i - 1][1].elapsed_time(seq[i][1])
    _MP["pend"].clear()
    n = _MP["n"]
    tot = sum(_MP["acc"].values())
    txt = ", ".join(k + " " + format(v / n, ".4f")
                    for k, v in sorted(_MP["acc"].items(), key=lambda kv: -kv[1]))
    init_logger(__name__).info(
        "glm53 MLA projection path: %d calls | marked total %.4f ms | %s", n, tot / n, txt)
