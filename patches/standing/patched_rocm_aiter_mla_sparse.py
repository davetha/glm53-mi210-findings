...
def fp8_paged_mqa_logits_torch(
    ...
    if next_n == 1:
        ...
        # gfx90a: this reference path is the PRODUCTION sparse-indexer kernel
        # (the AITER fp8 gate is False on CDNA2, which has no fp8 MFMA), so it
        # must be cudagraph-capturable. The upstream version reads
        # context_lens[i].item() and then slices by that scalar, which is both a
        # device->host sync per MLA layer and a data-dependent shape -- either
        # one invalidates capture with hipErrorStreamCaptureInvalidated.
        #
        # Take every page the block table can hold (a static bound) and mask the
        # out-of-context tail on device instead. Same result, no sync, static
        # shapes. Costs the max-context page count on short sequences, which is
        # a sub-millisecond gather here.
        max_pages = block_tables.shape[1]
        padded_seq_len = max_pages * block_size
        n_write = min(padded_seq_len, max_model_len)
        num_blocks_total = kv_cache_flat.shape[0]
        scale_offset = block_size * dim
        pos = torch.arange(padded_seq_len, device=q.device, dtype=torch.int32)
        for i in range(batch_size):
            q_i = q[i, 0].to(torch.float32)
            q_scale = weights[i]
            # Unused block-table slots hold NULL_BLOCK_ID / stale ids; clamp so
            # the gather stays in bounds. Their positions are masked out below.
            pages = block_tables[i].clamp(0, num_blocks_total - 1)
            ...
            score *= cache_scale
            score = torch.where(pos < context_lens[i], score, score.new_full((), float("-inf")))
            logits[i, :n_write] = score[:n_write]
        return logits
