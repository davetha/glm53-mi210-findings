...
class Glm5NextIndexerCache(DeepseekV32IndexerCache):
    ...
    *content* (softmax-weighted sum vs keep-every-Nth) is computed by the
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        if _cp.is_rocm() and storage_block_size % 16 == 0:
            # ROCm runs AITER's paged-MQA kernels, not DeepGEMM's, so the
            # (32, 64) page sizes below do not apply; AITER's constraint is the
            # 16-entry preshuffle tile. Taking the whole storage block keeps the
            # indexer's physical block equal to the model's, which leaves the
            # block-table reconciliation factor at 1.
            page_size = storage_block_size
        elif storage_block_size <= max_page_size:
            page_size = storage_block_size
