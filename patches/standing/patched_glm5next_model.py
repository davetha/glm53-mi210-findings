...
class Glm5NextMoE(nn.Module):
    ...
    def forward(
        ...
        if _MOE_TIMING:
            import torch as _t
            _MOE["n"] += 1
            _e = [_t.cuda.Event(enable_timing=True) for _ in range(3)]
            _e[0].record()
            ...
            _e[2].record()
            _MOE["pend"].append(_e)
            if _MOE["n"] % _MOE_TIMING == 0:
                from vllm.logger import init_logger
                _lg = init_logger(__name__)
                for _ev in _MOE["pend"]:
                    _ev[2].synchronize()
                    _MOE["gate"] += _ev[0].elapsed_time(_ev[1])
                    _MOE["experts"] += _ev[1].elapsed_time(_ev[2])
                _MOE["pend"].clear()
                _n = _MOE["n"]
                _lg.info(
                    "glm53 MoE module timing: %d calls | gate %.4f ms | experts %.4f ms",
                    _n, _MOE["gate"] / _n, _MOE["experts"] / _n)
        else:
            router_logits, _ = self.gate(hidden_states)
            final_hidden_states = self.experts(
                hidden_states=hidden_states, router_logits=router_logits
...
class Glm5NextDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if _HC_TIMING:
            return self._timed_mhc_forward(positions, hidden_states, residual, post, comb)
...
class Glm5NextModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if _LAYER_TIMING:
            hidden_states, residual, post, comb = _timed_layer_loop(
                self._active_layers, positions, hidden_states, residual, post, comb
            )
        else:
...
# --- layer timing diagnostic (GLM53_LAYER_TIMING=<report interval in steps>) ---
# Times each decoder layer with CUDA events bucketed by attention kind, so a
# decode step's cost lands in named buckets rather than being inferred from
# end-to-end step time. Events are recorded in the hot path and read only in
# bulk, so the measurement adds no per-layer sync.
import os as _os_lt

_LAYER_TIMING = int(_os_lt.environ.get("GLM53_LAYER_TIMING", "0"))
_LT = {"step": 0, "pend": [], "acc": {}, "cnt": {}}


def _layer_kind(layer) -> str:
    attn = getattr(layer, "self_attn", None)
    kind = type(attn).__name__ if attn is not None else "noattn"
    moe = "moe" if getattr(layer, "_mlp_is_moe", False) else "dense"
    return kind + "+" + moe


def _timed_layer_loop(layers, positions, hidden_states, residual, post, comb):
    import torch as _t

    _LT["step"] += 1
    evs = []
    e_prev = _t.cuda.Event(enable_timing=True)
    e_prev.record()
    for layer in layers:
        hidden_states, residual, post, comb = layer(
            positions, hidden_states, residual, post, comb
        ...
        e.record()
        evs.append((_layer_kind(layer), e_prev, e))
        e_prev = e
    _LT["pend"].append(evs)

    if _LT["step"] % _LAYER_TIMING == 0:
        from vllm.logger import init_logger

        lg = init_logger(__name__)
        for step_evs in _LT["pend"]:
            step_evs[-1][2].synchronize()
            for kind, a, b in step_evs:
                _LT["acc"][kind] = _LT["acc"].get(kind, 0.0) + a.elapsed_time(b)
                _LT["cnt"][kind] = _LT["cnt"].get(kind, 0) + 1
        _LT["pend"].clear()
        n = _LT["step"]
        tot = sum(_LT["acc"].values())
        parts = ", ".join(
            k + ": " + format(v / n, ".3f") + " ms/step over "
            + str(_LT["cnt"][k] // n) + " layers ("
            + format(v / _LT["cnt"][k], ".3f") + " ms/layer)"
            for k, v in sorted(_LT["acc"].items(), key=lambda kv: -kv[1])
        )
        lg.info("glm53 layer timing: %d steps | all-layers %.2f ms/step | %s",
                n, tot / n, parts)

    return hidden_states, residual, post, comb


# --- mHC decoder-layer sub-step timing (GLM53_HC_TIMING=<report interval>) ---
# Splits the mHC layer into hyper-connection pre/post, attention and MLP, to
# test whether hc_sinkhorn_iters=20 per layer is a material share of the decode
# step under forced-eager. Mirrors the mHC branch of forward() exactly.
_HC_TIMING = int(_os_lt.environ.get("GLM53_HC_TIMING", "0"))
_HC = {"n": 0, "pend": [], "acc": {}}
_HC_STEPS = ("hc_attn_pre", "attn", "hc_ffn_post_pre", "mlp", "hc_tail")


def _timed_mhc_forward(self, positions, hidden_states, residual, post, comb):
    import torch as _t

    _HC["n"] += 1
    ev = [_t.cuda.Event(enable_timing=True) for _ in range(len(_HC_STEPS) + 1)]
    i = 0
    ev[i].record(); i += 1
    ...
    out = (x, residual, post, comb)
    if self.layer_idx == self.num_hidden_layers - 1:
        x = self.hc_post(x, residual, post, comb)
        x = hc_contract(x, self.n)
        out = (x, None, None, None)
    ev[i].record()

    _HC["pend"].append(ev)
    if _HC["n"] % _HC_TIMING == 0:
        from vllm.logger import init_logger

        lg = init_logger(__name__)
        for e in _HC["pend"]:
            e[-1].synchronize()
            for j, name in enumerate(_HC_STEPS):
                _HC["acc"][name] = _HC["acc"].get(name, 0.0) + e[j].elapsed_time(e[j + 1])
        _HC["pend"].clear()
        n = _HC["n"]
        tot = sum(_HC["acc"].values())
        parts = ", ".join(
            k + " " + format(_HC["acc"][k] / n, ".4f") + " ms" for k in _HC_STEPS
        )
        lg.info("glm53 mHC layer timing: %d calls | total %.4f ms/call | %s",
                n, tot / n, parts)
    return out


Glm5NextDecoderLayer._timed_mhc_forward = _timed_mhc_forward


_MOE_TIMING = int(_os_lt.environ.get("GLM53_MOE_TIMING", "0"))
_MOE = {"n": 0, "pend": [], "gate": 0.0, "experts": 0.0}


# --- MoE runner whole-and-parts timing (GLM53_MK_TIMING=<report interval>) ---
# Lesson from the previous attempt: nested CUDA-event windows on a busy eager
# stream double-count queue stalls, and extra wrappers perturb the very numbers
# they measure. So this wraps the WHOLE (_forward_impl) in the SAME event stream
# as its PARTS (dispatch/routing, quant-method apply, combine/finalize). If the
# parts do not sum to the whole, the split is not believable and the log says so.
# Run this with EXPERT_CACHE_TIMING=0 -- one instrumentation layer at a time.
_MK_TIMING = int(_os_lt.environ.get("GLM53_MK_TIMING", "0"))

if _MK_TIMING:
    def _install_runner_timing():
        import torch as _t
        ...
        lg = init_logger(__name__)
        st = {"n": 0, "pend": [], "acc": {}, "whole": 0.0}
        parts = ("_maybe_dispatch", "_apply_quant_method", "_maybe_combine")

        def wrap_part(name):
            orig = getattr(_R, name)

            def timed(self, *a, **kw):
                e0 = _t.cuda.Event(enable_timing=True); e0.record()
                out = orig(self, *a, **kw)
                e1 = _t.cuda.Event(enable_timing=True); e1.record()
                st["pend"].append((name, e0, e1))
                return out

            timed.__name__ = name
            setattr(_R, name, timed)

        for nm in parts:
            if hasattr(_R, nm):
                wrap_part(nm)

        orig_impl = _R._forward_impl

        def timed_impl(self, *a, **kw):
            w0 = _t.cuda.Event(enable_timing=True); w0.record()
            out = orig_impl(self, *a, **kw)
            w1 = _t.cuda.Event(enable_timing=True); w1.record()
            st["pend"].append(("__whole__", w0, w1))
            st["n"] += 1
            if st["n"] % _MK_TIMING == 0:
                for nm, e0, e1 in st["pend"]:
                    e1.synchronize()
                    dt = e0.elapsed_time(e1)
                    if nm == "__whole__":
                        st["whole"] += dt
                    else:
                        st["acc"][nm] = st["acc"].get(nm, 0.0) + dt
                st["pend"].clear()
                n = st["n"]
                psum = sum(st["acc"].values())
                txt = ", ".join(
                    k + " " + format(v / n, ".4f")
                    for k, v in sorted(st["acc"].items(), key=lambda kv: -kv[1])
                )
                lg.info(
                    "glm53 runner timing: %d calls | whole %.4f ms | parts sum %.4f ms "
                    "(%.0f%% of whole) | %s",
                    n, st["whole"] / n, psum / n,
                    100.0 * psum / max(st["whole"], 1e-9), txt)
            return out

        _R._forward_impl = timed_impl

    try:
        _install_runner_timing()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("runner timing not installed: %s", _e)


# --- MLA sparse-attention split (GLM53_MLA_TIMING=<report interval>) ---
# Wraps the WHOLE MLA wrapper forward in the same event stream as its parts
# (sparse indexer, MLA attention) and prints parts-vs-whole. Every wrap happens
# ONCE at class level: wrapping per-instance re-wraps the shared class method
# for each of the 11 MLA layers and nests the timings 11 deep.
_MLA_TIMING = int(_os_lt.environ.get("GLM53_MLA_TIMING", "0"))

if _MLA_TIMING:
    def _install_mla_timing():
        import torch as _t
        from vllm.model_executor.layers.mla import (
            MultiHeadLatentAttentionWrapper as _W,
            MLAAttention as _A,
        )
        from vllm.models.glm5next.nvidia.attention import Indexer as _I
        from vllm.logger import init_logger

        lg = init_logger(__name__)
        st = {"n": 0, "pend": [], "acc": {}, "whole": 0.0}

        def wrap_once(cls, label):
            orig = cls.forward
            ...
            def timed(self, *a, **kw):
                ...
                out = orig(self, *a, **kw)
                e1 = _t.cuda.Event(enable_timing=True); e1.record()
                st["pend"].append((label, e0, e1))
                return out

            timed.__name__ = "forward"
            cls.forward = timed

        wrap_once(_A, "mla_attn")
        wrap_once(_I, "indexer")

        orig_fwd = _W.forward

        def timed_fwd(self, *a, **kw):
            w0 = _t.cuda.Event(enable_timing=True); w0.record()
            out = orig_fwd(self, *a, **kw)
            w1 = _t.cuda.Event(enable_timing=True); w1.record()
            st["pend"].append(("__whole__", w0, w1))
            st["n"] += 1
            if st["n"] % _MLA_TIMING == 0:
                for nm, e0, e1 in st["pend"]:
                    e1.synchronize(); dt = e0.elapsed_time(e1)
                    if nm == "__whole__": st["whole"] += dt
                    else: st["acc"][nm] = st["acc"].get(nm, 0.0) + dt
                st["pend"].clear()
                n = st["n"]; psum = sum(st["acc"].values())
                txt = ", ".join(k + " " + format(v / n, ".4f")
                                for k, v in sorted(st["acc"].items(), key=lambda kv: -kv[1]))
                lg.info("glm53 MLA timing: %d calls | whole %.4f ms | parts %.4f ms "
                        "(%.0f%% of whole) | %s",
                        n, st["whole"] / n, psum / n,
                        100.0 * psum / max(st["whole"], 1e-9), txt)
            return out

        _W.forward = timed_fwd

    try:
        _install_mla_timing()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("mla timing not installed: %s", _e)


# --- kernel-level decode profile (GLM53_PROF=<step to start>) ---
# CUDA-event instrumentation cannot see inside a captured graph: during replay
# the Python forward never runs. Hook GPUModelRunner.execute_model instead --
# that is called every step either way -- and let torch.profiler pick up the
# kernels the graph replay dispatches via roctracer. Profiles a fixed window
# after the model is warm, prints self-CUDA-time by kernel, then disarms.
_PROF_START = int(_os_lt.environ.get("GLM53_PROF", "0"))
_PROF_STEPS = int(_os_lt.environ.get("GLM53_PROF_STEPS", "12"))
...
if _PROF_START:
    def _install_prof():
        ...
        lg = init_logger(__name__)
        st = {"n": 0, "prof": None, "done": False}
        orig = _R.execute_model
        ...
        def timed(self, *a, **kw):
            ...
            st["n"] += 1
            if st["n"] == _PROF_START:
                st["prof"] = _t.profiler.profile(
                    activities=[
                        _t.profiler.ProfilerActivity.CPU,
                        _t.profiler.ProfilerActivity.CUDA,
                    ],
                    record_shapes=False, with_stack=False,
                )
                st["prof"].__enter__()
                lg.info("glm53 profile: started at step %d", st["n"])
            out = orig(self, *a, **kw)
            if st["prof"] is not None and st["n"] >= _PROF_START + _PROF_STEPS:
                st["prof"].__exit__(None, None, None)
                ka = st["prof"].key_averages()

                def _dev(e):
                    # attribute name moved across torch versions
                ...
                rows = sorted(ka, key=lambda e: -_dev(e))
                tot = sum(_dev(e) for e in ka)
                lg.info("glm53 profile: %d distinct events, device-time sum %.1f us",
                        len(ka), tot)
                n = _PROF_STEPS
                lg.info("glm53 profile: %d steps | total GPU %.2f ms/step", n, tot / 1000.0 / n)
                for e in rows[:28]:
                    us = _dev(e)
                    if us <= 0:
                        continue
                    lg.info("  %8.3f ms/step  %5.1f%%  n=%-6d %s",
                            us / 1000.0 / n, 100.0 * us / max(tot, 1), e.count, e.key[:78])
                st["prof"] = None
                st["done"] = True
            return out

        _R.execute_model = timed

    try:
        _install_prof()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("prof not installed: %s", _e)


# --- GLM53_DIE_AT_STEP=<n>: raise in execute_model after n steps (diagnostic only) ---
# The rocprofiler tool only writes its trace when the worker process exits normally;
# the executor shutdown path SIGKILLs workers after a 4 s grace, which loses it. A
# worker exception takes the orderly exit path (observed: trace flushed on the way out).
_DIE_AT = int(_os_lt.environ.get("GLM53_DIE_AT_STEP", "0"))
...
if _DIE_AT:
    def _install_die():
        ...
        def wrapped(self, *a, **kw):
            st["n"] += 1
            if st["n"] > _DIE_AT:
                # SystemExit, not RuntimeError: the busy loop catches Exception and keeps the
                # worker alive (it then gets SIGKILLed, and the tracer never flushes).
                raise SystemExit(f"glm53: intentional worker exit after step {_DIE_AT}")
            return orig(self, *a, **kw)

        _R.execute_model = wrapped

    try:
        _install_die()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("die-at-step not installed: %s", _e)


# --- expert routing audit (GLM53_ROUTE_AUDIT=<report interval in calls>) ---
# Explains the per-rank hit-rate gap (TP0 34% vs TP1 55%). The slot cache's hit
# rate is essentially the consecutive-step overlap of the routed expert set, so
# measure that directly, per layer, alongside the access histogram. Eager only:
# this lives in a Python wrapper and never runs during graph replay.
_ROUTE_AUDIT = int(_os_lt.environ.get("GLM53_ROUTE_AUDIT", "0"))

if _ROUTE_AUDIT:
    def _install_route_audit():
        import torch as _t
        ...
        lg = init_logger(__name__)
        st = {"n": 0, "prev": {}, "ov": 0.0, "ovn": 0, "hist": None, "lo": 10**9, "hi": -1}
        orig = _R.forward_modular

        def timed(self, x, topk_weights, topk_ids, *a, **kw):
            try:
                ids = topk_ids.detach().flatten()
                key = id(self)
                cur = set(ids.tolist())
                if st["hist"] is None:
                    st["hist"] = {}
                for e in cur:
                    st["hist"][e] = st["hist"].get(e, 0) + 1
                    if e < st["lo"]: st["lo"] = e
                    if e > st["hi"]: st["hi"] = e
                prev = st["prev"].get(key)
                if prev is not None and cur:
                    st["ov"] += len(cur & prev) / len(cur)
                    st["ovn"] += 1
                st["prev"][key] = cur
                st["n"] += 1
                if st["n"] % _ROUTE_AUDIT == 0:
                    h = st["hist"]
                    tot = sum(h.values())
                    vals = sorted(h.values(), reverse=True)
                    top10 = sum(vals[: max(1, len(vals) // 10)]) / max(tot, 1)
                    lg.info(
                        "glm53 route audit: %d calls | consecutive-step overlap "
                        "%.1f%% | distinct experts %d (ids %d..%d) | top-10%% of "
                        "experts take %.1f%% of routes",
                        st["n"], 100.0 * st["ov"] / max(st["ovn"], 1),
                        len(h), st["lo"], st["hi"], 100.0 * top10)
            except Exception:
                pass
            return orig(self, x, topk_weights, topk_ids, *a, **kw)

        _R.forward_modular = timed

    try:
        _install_route_audit()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("route audit not installed: %s", _e)


# --- quant-method resolution trace (GLM53_QTRACE=1) ---
# Three load attempts of the int8-attention checkpoint died on
# KeyError: layers.0.self_attn.in_proj_qkvbfg_a.weight_scale, meaning the module
# was built UNQUANTIZED even though an offline replay of the matcher showed it
# is neither ignored nor unmatched. Log what get_quant_method actually returns
# for the attention prefixes, and why.
_QTRACE = int(_os_lt.environ.get("GLM53_QTRACE", "0"))

if _QTRACE:
    def _install_qtrace():
        from vllm.model_executor.layers.quantization.compressed_tensors.compressed_tensors import (  # noqa: E501
            CompressedTensorsConfig as _C,
        )
        from vllm.logger import init_logger

        lg = init_logger(__name__)
        orig = _C.get_quant_method
        seen = set()

        def traced(self, layer, prefix):
            out = orig(self, layer, prefix)
            key = prefix.split(".")[-1]
            if ("self_attn" in prefix or "attn" in prefix) and key not in seen:
                seen.add(key)
                scheme = None
                try:
                    scheme = type(self.get_scheme(layer, prefix)).__name__
                except Exception as e:
                    scheme = f"<get_scheme raised {type(e).__name__}: {e}>"
                lg.info(
                    "QTRACE prefix=%s cls=%s -> method=%s scheme=%s",
                    prefix, type(layer).__name__,
                    type(out).__name__ if out is not None else "None", scheme)
            return out

        _C.get_quant_method = traced

    try:
        _install_qtrace()
    except Exception as _e:
        from vllm.logger import init_logger as _il
        _il(__name__).warning("qtrace not installed: %s", _e)


# --- decode fast path (GLM53_FASTPATH=0 to disable): see glm53_fastpath.py next to this file ---
try:
    from . import glm53_fastpath as _glm53_fastpath
    _glm53_fastpath.install()
    if _os_lt.environ.get("GLM53_COMPILE", "0") == "1":
        _glm53_fastpath.install_compile()
except Exception as _e:
    from vllm.logger import init_logger as _il
    _il(__name__).error("glm53 fastpath NOT installed -- running the slow path: %s", _e, exc_info=True)
