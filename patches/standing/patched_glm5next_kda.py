...
class Glm5NextLinearAttention(GatedDeltaNetAttention):
    ...
    def forward(
        ...
        if _KDA_TIMING:
            return self._timed_forward(hidden_states, positions)
...
# --- KDA sub-step timing (GLM53_KDA_TIMING=<report interval in calls>) ---
# Splits the KDA block into projection / small-proj / core / norm / out_proj so
# the 1.6 ms-per-layer decode cost lands on a specific sub-step. Mirrors
# forward() exactly; CUDA events are recorded hot and read only in bulk.
import os as _os_kt

_KDA_TIMING = int(_os_kt.environ.get("GLM53_KDA_TIMING", "0"))
_KT = {"n": 0, "pend": [], "acc": {}}
_KT_STEPS = ("in_proj", "small_proj", "core", "o_norm", "o_proj")


def _kda_timed_forward(self, hidden_states, positions):
    import torch as _t

    _KT["n"] += 1
    evs = [_t.cuda.Event(enable_timing=True) for _ in range(len(_KT_STEPS) + 1)]
    i = 0
    evs[i].record(); i += 1
    ...
    self._forward(
        qkv_proj_states=qkv, g1=g1, beta=beta, core_attn_out=core_attn_out
    )
    evs[i].record(); i += 1
    ...
    _KT["pend"].append(evs)
    if _KT["n"] % _KDA_TIMING == 0:
        from vllm.logger import init_logger

        lg = init_logger(__name__)
        for e in _KT["pend"]:
            e[-1].synchronize()
            for j, name in enumerate(_KT_STEPS):
                _KT["acc"][name] = _KT["acc"].get(name, 0.0) + e[j].elapsed_time(e[j + 1])
        _KT["pend"].clear()
        n = _KT["n"]
        tot = sum(_KT["acc"].values())
        parts = ", ".join(
            k + " " + format(_KT["acc"][k] / n, ".4f") + " ms"
            for k in _KT_STEPS
        )
        lg.info("glm53 KDA sub-step timing: %d calls | total %.4f ms/call | %s",
                n, tot / n, parts)
    return out


Glm5NextLinearAttention._timed_forward = _kda_timed_forward
