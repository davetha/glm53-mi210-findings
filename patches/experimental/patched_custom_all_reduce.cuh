                        ...
                        int rank)
{
    ...
    if(threadIdx.x < ngpus)
    {
        ...
        __scoped_atomic_store_n(&sg.signals[threadIdx.x]->start[blockIdx.x][rank],
                                flag,
                                __ATOMIC_RELEASE,  // gfx90a: PCIe gives no free ordering
                                __MEMORY_SCOPE_SYSTEM);
        // wait until we got true from all ranks
        // Poll relaxed; acquire ONCE below. Acquire-per-iteration costs PCIe coherence
        // traffic every time round and measured ~11 ms/step worse than NCCL.
