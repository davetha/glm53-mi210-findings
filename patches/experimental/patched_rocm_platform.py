...
class RocmPlatform(Platform):
    ...
    def use_custom_allreduce(cls) -> bool:
        # Upstream allowlists MI300-series only. That is a vendor decision, not a capability
        # test: the checks in custom_all_reduce.py would PASS on a 2-GPU gfx90a box (the
        # PCIe-only restriction applies only above two GPUs, and the P2P probe is skipped
        # entirely on ROCm), and _C_stable_libtorch.abi3.so in this image carries 50 gfx90a
        # code objects including init_custom_ar.
        #
        # Why it matters here: on 2x MI210, NCCL costs 10.5 ms of an 80 ms decode step
        # (92 calls x 113.7 us). These cards have no XGMI and sit on different IO-die
        # quadrants, so every reduction crosses PCIe.
        #
        # OPT-IN ONLY. The kernel may assume Infinity Fabric ordering guarantees that PCIe
        # peer-to-peer does not provide, which would corrupt output rather than crash.
        # Verify generated text before trusting any timing.
        allow = ["gfx94", "gfx95"]
