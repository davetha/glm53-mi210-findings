...
class rocm_aiter_ops:
    ...
    def is_custom_all_reduce_enabled(cls) -> bool:
        # Deliberately NOT @if_aiter_supported. That umbrella requires CDNA 3+, and
        # widening it turns on every AITER path -- on gfx90a that reaches Triton GEMMs
        # with no tuning config and kills startup. The all-reduce itself works on CDNA2
        # once its peer handshake uses release/acquire at system scope (see
        # patched_custom_all_reduce.cuh), so exempt just this one.
        from vllm.platforms.rocm import on_gfx90a
        if not (is_aiter_found_and_supported() or (IS_AITER_FOUND and on_gfx90a())):
            return None
