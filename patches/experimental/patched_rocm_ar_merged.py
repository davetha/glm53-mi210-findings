...
def _query_gcn_arch_from_amdsmi() -> str:
    ...
    amdsmi enumerates PHYSICAL devices and ignores HIP_VISIBLE_DEVICES, so on a
    machine holding more than one GPU architecture, handles[0] can name a card
    this process was never given. Observed on a host with 2x MI210 (gfx90a) plus
    2x R9700 (gfx1201): physical 0 is an R9700, so every CDNA gate silently
    turned off while torch correctly reported gfx90a, costing the AITER int8
    GEMM path (2.35x decode).

    Only the homogeneous case is safe to answer from amdsmi. When the visible
    architectures disagree, raise so the caller falls back to torch.cuda, which
    does honour the visibility mask.
    """
    ...
    archs = []
    for h in handles:
        gfx = amdsmi_get_gpu_asic_info(h).get("target_graphics_version", "")
        if gfx:
            archs.append(gfx)
    if not archs:
        raise RuntimeError("amdsmi did not return valid GCN arch")
    if len(set(archs)) > 1:
        raise RuntimeError(
            f"amdsmi sees mixed GPU architectures {sorted(set(archs))}; "
            "cannot pick one without the visibility mask"
        )
    return archs[0]
...
class RocmPlatform(Platform):
    ...
    def use_custom_allreduce(cls) -> bool:
        # Upstream allowlists MI300-series. That gates BOTH vLLM's own custom kernel and
        # the AITER one, because cuda_communicator computes
        #     use_aiter_allreduce = use_custom_allreduce and is_custom_all_reduce_enabled()
        # so AITER stays unreachable on gfx90a even with the _aiter_ops CDNA2 patch applied.
        #
        # Motivation: on 2x MI210 with no XGMI and the cards on separate IO-die quadrants,
        # NCCL costs 10.5 ms of an 80 ms decode step (92 calls x 113.7 us).
        #
        # NOTE vLLM's OWN custom kernel is known broken here -- it fails at
        # custom_all_reduce_hip.cuh:167 'invalid argument' during graph capture on gfx90a.
        # This opt-in exists to reach the AITER path, which is selected in preference to it
        # (cuda_communicator only builds ca_comm when aiter_ar_comm is None).
        allow = ["gfx94", "gfx95"]
        if os.environ.get("VLLM_ALLOW_CUSTOM_AR_GFX90A", "0") != "0":
            allow.append("gfx90a")
        return any(gfx in _GCN_ARCH for gfx in allow)
