...
def _deepgemm_fp8_paged_mqa_logits_stage1_ragged_k(
    ...
    for context_idx in range(
        ...
        # gfx90a (CDNA2) has no fp8 MFMA, so Triton refuses fp8 operands to
        # tl.dot. Upcast to bf16 first: every fp8e4m3 value is exactly
        # representable in bf16 (the e4m3 range and 3-bit mantissa both fit),
        # so this is lossless -- the dot already accumulates in fp32.
        o = tl.dot(q.to(tl.bfloat16), k.to(tl.bfloat16).T)
        o = o * k_scale_f.T
        o = tl.maximum(o, 0.0)
