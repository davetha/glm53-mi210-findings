...
#else
// gfx90a (CDNA2) has no fp8-conversion-insts; v_cvt_pk_fp8_f32 does not assemble.
// Nothing on the all-reduce path converts FP8, so these exist only to let the header
// parse. They trap rather than return wrong values.
template<typename S, index_t sel = 0, std::enable_if_t<std::is_same_v<S, fp32x2_t>, bool> = true>
OPUS_D decltype(auto) fp32_to_fp8_packed_x2(const S&, number<sel> = {}) {
    __builtin_trap(); return fp8x2_t{};
}
template<typename S, std::enable_if_t<std::is_same_v<S, fp32x4_t>, bool> = true>
OPUS_D decltype(auto) fp32_to_fp8_packed_x4(const S&) {
    __builtin_trap(); return fp8x4_t{};
}
template<typename S, index_t sel = 0, std::enable_if_t<std::is_same_v<S, fp8x2_t>, bool> = true>
OPUS_D decltype(auto) fp8_to_fp32_packed_x2(const S&, number<sel> = {}) {
    __builtin_trap(); return fp32x2_t{};
}
template<typename S, std::enable_if_t<std::is_same_v<S, fp8x4_t>, bool> = true>
OPUS_D decltype(auto) fp8_to_fp32_packed_x4(const S&) {
    __builtin_trap(); return fp32x4_t{};
}
#endif
